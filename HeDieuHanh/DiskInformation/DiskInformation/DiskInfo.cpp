...
{	    
	this->ReadLogicalPartitionsInfo(); // goi ham doc thong tin cac o dia
	this->ReadHardisksInfo();		   // goi ham doc thong tin cac phan vung
}

//Doc thong tin o dia
// voi moi thong tin doc duoc, tien hanh them vao danh sach thog tin o cung
// su dung API cua WIN de ho tro
	...
	for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
	{
		...
		if (hPhysicalDrive != INVALID_HANDLE_VALUE)
		{
			...
			{         
				...
				hardiskInfo->vendorId = string(vendorId);		// ten nha san xuat
				hardiskInfo->productId = string(modelNumber);   // ma san pham
				hardiskInfo->serialNumber = string(serialNumber);// so hieu san pham
				hardiskInfo->bytePerSector = 0;			// so byte/sector ( thuong la 512 )
				hardiskInfo->cylinders = 0;				// so cylinder co trong o cung
				hardiskInfo->diskSize = 0;				// dung luong o cung
				hardiskInfo->tracksPerCylinder = 0;		//so tracks/cylinder
				hardiskInfo->sectorsPerTrack = 0;		// so sectors/track
				hardiskInfo->driverType = -1;			// kieu o cung
				listHardiskInfo.push_back(hardiskInfo); // day vao struct
...
//Doc thong tin cua Logitcal partition.
// voi moi thogn tin phan vung doc duoc, them vao danh sach cac phan vung
// su dung API cua WIN de ho tro
...
::GetLogicalDriveStrings(lenght, names);	
    ...
    {        
		...
		unsigned long bytesPerSector = 0;		// so bytes/sector
		unsigned long sectorsPerCluster = 0;	// so sectors/cluster
		unsigned long numberOfFreeClusters = 0;	// so clusters chua su dung ( dung luong con trong cua phan vung)
		unsigned long totalNumberOfClusters = 0;// tong so clusters ( dung luong cua phan vung)
		unsigned long volumeSerialNumber = 0;	// so hieu phan vung logic
